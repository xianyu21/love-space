    ...
    function submit() {
      ...
      common_vendor.index.$tm.u.throttle(
        () => {
          const result = validate();
          validateResultList.value = [...result.result];
          emits("submit", { data: common_vendor.toRaw(_modelVal.value), ...result });
        },
        220,
        false
      );
      ...
      for (let i = 0, len = par.length; i < len; i++) {
        ...
        for (let j = 0; j < vallist.length; j++) {
          if (!vallist[j].validator) {
            isPass = false;
            rulstVal.message = ((_a = vallist[j]) == null ? void 0 : _a.message) ?? "校验通过";
            rulstVal.validator = ((_b = vallist[j]) == null ? void 0 : _b.validator) ?? true;
            break;
          }
        }
...
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__file", "C:/Users/admin/Desktop/github/love-space/src/tmui/components/tm-form/tm-form.vue"]]);
